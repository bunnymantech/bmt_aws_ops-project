...
------------------------------------------------------------------------------
``bmt_eco_scheduler/config/define/`` 模块定义了 Config Schema. 我们将众多的 Config value 分为两类:
...
Config Loading
------------------------------------------------------------------------------
``bmt_eco_scheduler/config/load.py`` 模块负责实例化 Config 对象.
...
1. 如果你要修改 Config Declaration 的定义, 在 ``bmt_eco_scheduler/config/define/`` 模块中进行修改.
2. 如果你要修改 Config 的值, 在本地电脑上修改 ``config/config.json`` 和 ``${HOME}/.projects/bmt_eco_scheduler/config-secret.json`` 文件.
