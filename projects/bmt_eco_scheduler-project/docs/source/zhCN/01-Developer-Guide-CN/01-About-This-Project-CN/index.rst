...
.. dropdown:: bmt_eco_scheduler/env.py

    .. literalinclude:: ../../../../bmt_eco_scheduler/env.py
       :language: python
       :linenos:

**Semantic Git Branching**

在这个项目中, 我们使用了 `aws_ops_alpha.api.bmt_eco_scheduler_project <https://aws-ops-alpha.readthedocs.io/en/latest/zhCN/02-Code-Recipes-CN/04-Rule-Set-Code-Recipe-CN/index.html#simple-lambda-rule-set>`_ 默认的策略.


DevOps CLI Tool
------------------------------------------------------------------------------
当 CD 到 ``bmt_eco_scheduler-project`` 目录之后, 你可以用 ``make ${command}`` 命令来执行常见的 DevOps Step. 你也可以用 ``make`` 命令列出所有的 Command.
