...
The ``cdk`` folder
------------------------------------------------------------------------------
The ``cdk`` folder is the CDK official convention where you should store your IAC declaration code and where you should run the ``cdk deploy`` command. However, for enterprise project, we should break down complicated logics into modules for better maintainability. The ``cdk/app.py`` script just import the declaration code from the ``bmt_eco_scheduler/iac/`` Python modules.


The ``bmt_eco_scheduler/iac/`` Python Modules
------------------------------------------------------------------------------
``bmt_eco_scheduler/iac/`` is the root folder of the infrastructure as code source code. Below are the list of important files related to Infrastructure as Code::

    bmt_eco_scheduler/iac # the root folder of the infrastructure as code source code
    bmt_eco_scheduler/iac/define/ # CDK stack declaration code
    bmt_eco_scheduler/iac/define/main.py # centralized CDK stack object, AWS resources are break down into sub-modules
    bmt_eco_scheduler/iac/define/iam.py # IAM related AWS resources
    bmt_eco_scheduler/iac/define/lbd.py # Lambda function related AWS resources
    bmt_eco_scheduler/iac/exports.py # CDK stack output exports for other projects to use
...
------------------------------------------------------------------------------
1. Working on ``bmt_eco_scheduler/iac/`` module to declare your CDK stack.
