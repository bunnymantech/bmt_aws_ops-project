...
First, let's create a new project code skeleton using a sample project. It will prompt you to enter your project name and some configuration::
    ...
    # assume that you are currently at the ``./projects/bmt_eco_scheduler-project`` directory.
    python ../../cookiecutter/bmt_eco_scheduler.py
...
Now that you have a new project to work on, the project name is the one you just entered. However, for illustrative purposes, let's assume the project name is also ``bmt_eco_scheduler`` (``${du_name}``). Please note that all the following documentation is based on this assumption. In your actual project development, make sure to replace the name with your real project name.

Every project requires a collection of AWS resources for CI/CD. To provision these resources, we need to run a script called ``bootstrap``. Please refer to the ``./projects/bmt_eco_scheduler-project/bootstrap/README.rst`` file for more details. You can use the following command to provision the CI/CD AWS resource::
...
------------------------------------------------------------------------------
In this step, we focus on the core application code logic of the project. From this point, I assume that you are currently at the ``./projects/bmt_eco_scheduler-project`` (``./projects/${du_name}-project``) directory.

1. Create a feature branch: ``bmt_eco_scheduler/feature/${description}`` (``${du_name}/feature/${description}``) and start development.
...
10. Now you can merge the ``bmt_eco_scheduler/feature/${description}`` to ``main``.
...
1. Create a layer branch ``bmt_eco_scheduler/layer/${description}`` (``${du_name}/layer/${description}``).
...
4. Once you see a new layer is published, you can merge the ``bmt_eco_scheduler/layer/${description}`` to ``main``.
...
1. Create a lambda branch ``bmt_eco_scheduler/lambda/${description}`` (``${du_name}/lambda/${description}``).
...
6. Once you see the app is deployed to ``sandbox`` and the integration test is passed, you can merge the ``bmt_eco_scheduler/lambda/${description}`` to ``main``.
...
1. Create a release branch ``bmt_eco_scheduler/release`` (``${du_name}/release``). Don't do any application code change in this branch. If you have to, please roll back to the previous step to ensure that your change doesn't break the test.
...
- Create a cleanup branch: ``bmt_eco_scheduler/cleanup`` (``${du_name}/cleanup``). Don't do any application code change in this branch.
